...
class Scope {//作用域，类似静态语义检查实验的工作量
  public:
    // enter a new scope
...
class CminusfBuilder : public ASTVisitor {//访问者模式
  public:
    CminusfBuilder() {
  ...
  private:
    ...
    struct {//存储翻译时状态，可以是当前正在处理的函数等信息
        // function that is being built
        Function *func = nullptr;
        unsigned label = 0;//标识最新的未被使用的label
        Value *value = nullptr;//临时存储的值
        bool is_lvalue = 0;//是否是左值
    } context;
};
