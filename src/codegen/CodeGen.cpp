...
void CodeGen::gen_epilogue() {
    ...
    auto exit_label = context.func->get_name() + "_exit";  
    ...
    //int stack_size = context.frame_size;
...
void CodeGen::gen_ret() {
    ...
    if (retInst->is_void_ret()) {
        append_inst("addi.d $a0, $zero, 0");  // 题目规范，若返回值为void类型，则默认0
    } else if (retInst->get_operand(0)->get_type()->is_float_type()) {//返回float
